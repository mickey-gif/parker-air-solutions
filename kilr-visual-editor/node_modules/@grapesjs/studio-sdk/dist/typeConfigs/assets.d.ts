...
import { StudioLayoutComponentsConfig } from '../components/public/types';
import type { AssetManagerBaseProps } from '../components/public/types/StudioPanelAssetsSchema';
import { BaseAssetProvider } from './assetsSchema';
...
export interface AssetProps {
    ...
    mimeType?: string;
    /**
     * The asset type. If not present, it's extracted from mimeType, otherwise, defaults to `image`.
     */
    type?: string;
    /**
...
export interface AssetProvider extends Omit<BaseAssetProvider, 'types' | 'onLoad' | 'itemLayout'> {
    /**
     ...
     * 'My asset provider'
     *
     * ({ editor }) => editor.I18n.t('myProviderLabel')
     */
    label: string | ((props: WithEditorProps) => string);
    /**
     * Icon to display in the asset provider filter.
     */
    icon?: string;
    /**
 ...
 */
export interface AssetManagerProps extends Omit<AssetManagerBaseProps, 'layoutAfterHeader'> {
    /**
    ...
    /**
     * Custom layout configuration for additional header content in the asset manager.
     * @example
     * layoutAfterHeader: ({ editor }) => ({
     *   type: 'row',
     *   style: { padding: '8px 16px', borderBottom: '1px solid #e0e0e0' },
     *   children: [
     *     {
     *       type: 'button',
     *       variant: 'outline',
     *       content: 'Custom Action',
     *       onClick: () => editor.runCommand('custom:action')
     *     }
     *   ]
     * })
     */
    layoutAfterHeader?: ({ editor }: {
        editor: Editor;
    }) => StudioLayoutComponentsConfig;
}
export type CustomAssetManagerProps = AssetsResultProps & Omit<AssetManagerProps, 'panelId' | 'select'> & {
    open?: (props: any) => void;
    close?: (props: any) => void;
};
export interface AssetsConfig extends Pick<AssetManagerProps, 'providerId' | 'projectAssetsOption' | 'layoutAfterHeader'> {
    /**
