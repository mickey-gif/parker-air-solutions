...
$(document).ready(() => {
    ...
    async function loadCartContents() {
        const cookieValue = await Wized.data.c.cart;
        if (cookieValue === undefined) {
            ...
            try {
                ...
                console.error("Error parsing c.cart contents: ", e);
                cartContents = [];
            }
        }
    ...
    async function renderCartVariants() {
        ...
        $cartVariants.each((index, cartVariant) => {
            ...
            $(cartVariant).find('[kilr_shopify="cart_quantity-dec"]').click(async () => {
                if (cartContents[index].qty > 1) {
                    ...
                    console.log('Trying to reveal the delete confirmation...');
                    $(cartVariant).find('[kilr_shopify="cart_variant_delete-alert"]').css('display', 'flex');
                    console.log('Delete confirmation should be visible now.');
                }
            });
        ...
        const currentCartContents = await Wized.data.c.cart;
        console.log("c.cart contents:", currentCartContents);
        console.log("cartContents array:", cartContents);
    ...
    async function updateWizedCheckoutVariable() {
        ...
        cartContents.forEach((item, index) => {
            // Start of the item object
            gqlFormattedString += '{';

            // Add variantId and quantity without quotes on the keys
            gqlFormattedString += `variantId:"${item.gid}",quantity:${item.qty},`;

            // Start of the customAttributes array
            gqlFormattedString += 'customAttributes:[';

            // Add each attribute object
            item.customAttributes.forEach((attr, attrIndex) => {
                gqlFormattedString += `{key:"${attr.key}",value:"${attr.value}"}`;
                // If not the last attribute, add a comma
                if (attrIndex < item.customAttributes.length - 1) {
                    gqlFormattedString += ',';
                }
            });

            // End of the customAttributes array
            gqlFormattedString += ']';
