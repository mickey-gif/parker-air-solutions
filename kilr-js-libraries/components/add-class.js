 ...
 * Add Class Handler
 * Version: 1.0.8
 * Description: Adds/removes classes to elements based on hover, click, click-out, or scroll triggers (in-25, in-25-out-75) with flexible breakpoint support. Supports group mode for exclusive class application. Supports list mode with on-load and add-mode (checkbox/radio) functionality.
...
(function() {
    class AddClassHandler {
        ...
        constructor() {
            ...
            this.lists = new Map(); // Store list information
        ...
        initializeLists() {
            ...
            const listContainers = document.querySelectorAll('[kilr-add-class="list"]');
            
            listContainers.forEach(listContainer => {
                // Find all trigger elements within this list
                const listTriggers = listContainer.querySelectorAll('[kilr-add-class="trigger"]');
                const listId = this.generateGroupId(); // Reuse same ID generator
                
                // Get list configuration
                const onLoadIndex = listContainer.getAttribute('data-on-load');
                let addMode = listContainer.getAttribute('data-add-mode') || 'radio'; // Default to radio
                
                if (addMode !== 'checkbox' && addMode !== 'radio') {
                    console.warn(`AddClassHandler: Invalid data-add-mode "${addMode}". Must be "checkbox" or "radio". Defaulting to "radio".`);
                    addMode = 'radio';
                }
                ...
                const listData = {
                    ...
                    triggers: Array.from(listTriggers),
                    addMode: addMode,
                    className: null // Will be set from first trigger
                };
                
                // Initialize list triggers and get common className
                listTriggers.forEach((trigger, index) => {
                    const className = trigger.getAttribute('data-class-name');
                    if (className) {
                        if (!listData.className) {
                            listData.className = className;
                        }
                        // Store list info on each trigger
                        trigger._addClassListId = listId;
                        trigger._addClassListData = listData;
                    }
                });
                
                if (listData.className && listData.triggers.length > 0) {
                    this.lists.set(listId, listData);
                    
                    // Apply on-load class if specified
                    if (onLoadIndex !== null && onLoadIndex !== '') {
                        const index = parseInt(onLoadIndex, 10);
                        if (!isNaN(index) && index >= 1 && index <= listData.triggers.length) {
                            // Apply class to the specified element (1-indexed)
                            const targetElement = listData.triggers[index - 1];
                            targetElement.classList.add(listData.className);
                        } else {
                            console.warn(`AddClassHandler: Invalid data-on-load value "${onLoadIndex}". Must be between 1 and ${listData.triggers.length}.`);
                        }
                    }
                }
            });
        ...
        handleGroupExclusivity(element, className) {
            ...
            // Check if this element is part of a list with radio mode
            if (element._addClassListId && element._addClassListData) {
                const listData = element._addClassListData;
                // Only enforce exclusivity if in radio mode
                if (listData.addMode === 'radio') {
                    listData.triggers.forEach(trigger => {
                        if (trigger !== element && trigger.classList.contains(className)) {
                            trigger.classList.remove(className);
                        }
                    });
        ...
        setupClickTrigger(element, className) {
            const toggleClass = () => {
                ...
                // Check if element is in a list with checkbox mode
                if (element._addClassListId && element._addClassListData) {
                    const listData = element._addClassListData;
                    if (listData.addMode === 'checkbox') {
                        // Checkbox mode: simple toggle, no exclusivity
                        if (hasClass) {
                            element.classList.remove(className);
                        } else {
                            element.classList.add(className);
                ...
                // Default behavior (radio mode or group mode or standalone)
                if (hasClass) {
                    element.classList.remove(className);
                } else {
                    this.handleGroupExclusivity(element, className);
        ...
        setupScrollTrigger(element, className, scrollIn, scrollOut) {
            ...
            const scrollHandler = () => {
                ...
                // visibleFromTop = how many pixels of the element are visible from the top
                ...
                if (elementBottom <= 0) {
                    ...
                    // Visible amount = how much has scrolled past the top (but not more than element height)
                    visibleFromTop = Math.min(elementHeight, -elementTop + Math.min(viewportHeight, elementHeight));
                } else {
                ...
                const visiblePercentage = elementHeight > 0 ? (visibleFromTop / elementHeight) * 100 : 0;
            ...
            // Use IntersectionObserver to detect when element enters/leaves viewport
            const observerCallback = (entries) => {
                entries.forEach(entry => {
                    // Trigger scroll handler when element visibility changes
                    scrollHandler();
                });
            };
